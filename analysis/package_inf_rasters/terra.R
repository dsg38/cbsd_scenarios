...
polyDfPath = here::here("inputs/inputs_raw/polygons/polys_2_host_default.gpkg")
...
for(i in seq_len(dim(rasterStackNumFields)[[3]])){
    ...
    a = dplyr::full_join(rasterPolySumDf, rasterPolyNumPopulatedDf, by="GID_0")
    
    rasterDfList[[rasterName]] = dplyr::full_join(rasterPolySumDf, rasterPolyNumPopulatedDf, by="GID_0")
...
for(rasterPath in rasterPaths){
    ...
    rasterStatsPolyDfPartial = dplyr::full_join(polyDf, rasterStatsDf, by="GID_0")
...
outPath = here::here(jobDir, "results_poly_stats.rds")
print(outPath)
