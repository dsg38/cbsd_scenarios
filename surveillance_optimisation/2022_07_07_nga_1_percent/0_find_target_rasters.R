...
polyDfSubset = polyDfSplitList[["NGA"]]
...
for(thisSimKey in simKeysAnyPos){
    ...
    polyDfAnyPos = polyDfSubset |>
        ...
        dplyr::filter(raster_prop_fields > 0.01)
    
    if(nrow(polyDfAnyPos)>=1){
        dpcDfList[[thisSimKey]] = polyDfAnyPos[1,] # Save only first 5 years
    }
...
# hist(dpcDf$raster_prop_fields, xlim=c(0, 1))
